...
banner: "Trackers and Galleries/Things/Attachments/Banners/movies.jpg"
cssClasses: cards, cards-cover, cards-2-3, table-max, max
banner_y: 0.55422
---
```dataviewjs
let MovieCount = (dv.pages('"Y Movies and Games"').where(p => p.Medium == "Movie").length);

let TVCount = (dv.pages('"Y Movies and Games"').where(p => p.Medium == "TV").length);

let GameCount = (dv.pages('"Y Movies and Games"').where(p => p.Medium == "Game").length);


let durationsum = 0;
	for(let i = 0; i < dv.pages('"Y Movies and Games"').length;i++) {
     if(dv.pages('"Y Movies and Games"')[i].Length) {
       durationsum += dv.pages('"Y Movies and Games"')[i].Length;
         }
   }

let gametime = 0;
	for(let i = 0; i < dv.pages('"Y Movies and Games"').where(p => p.Medium == "Game").length;i++) {
     if(dv.pages('"Y Movies and Games"').where(p => p.Medium == "Game")[i].Length) {
       gametime += dv.pages('"Y Movies and Games"').where(p => p.Medium == "Game")[i].Length;
         }
   }

let Grade = 0;
	for(let i = 0; i < dv.pages('"Y Movies and Games"').length;i++) {
     if(dv.pages('"Y Movies and Games"')[i].Rating) {
       Grade += dv.pages('"Y Movies and Games"')[i].Rating;
         }
   }
...
dv.paragraph("#### Since 2022, I've watched " + MovieCount + " movies and I've watched to " + TVCount + " TV shows, for a total of " + durationsum.toFixed() + " minutes, or " + (durationsum / 60).toFixed(2) + " hours, of which " + (gametime/60) + " hours were games being played.")
...
```dataviewjs
	...
	for(
		let results of dv.pages('"Y Movies and Games"')
...
for(
	...
	for(
		let results of dv.pages('"Y Movies and Games"')
			.where(p => p.DateFinished && p.Medium == "Game") 
			.sort(p => p.DateFinished, 'asc')
			.groupBy(p => (dv.date(p.DateFinished).toFormat("MMMM yyyy")))
			.sort(ym => ym.rows.DateFinished.first(), 'asc')
			.groupBy(ym => (dv.date(ym.rows.DateFinished.first()).year))
			...
			let lbl = "Games completed in " + results.key;
			let backCol = colors[datasets.length%colors.length];
			let bordCol = colors[datasets.length%colors.length];
			let bWidth = 1;
...
data: |
  dataviewjs:
  return dv.pages('"Y Movies and Games"')
  .where(p => p.DateFinished)
