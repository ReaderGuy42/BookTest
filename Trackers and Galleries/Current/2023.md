...
```dataviewjs
let BookCount = (dv.pages('"Book Log/2023"').where(p => p.Medium == "Book" || p.Medium == "eBook" || p.Medium == "GraphicNovel").length);

let AudioCount = (dv.pages('"Book Log/2023"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast").length);

let AcademicCount = (dv.pages('"Book Log/2023"').where(p => p.Medium == p.Medium == "AcademicBook" || p.Medium == "AcademicArticle").length);

let pagesum = 0;
   for(let i = 0; i < dv.pages('"Book Log/2023"').where(p => p.Medium == "Book" || p.Medium == "eBook" || p.Medium == "GraphicNovel" || p.Medium == p.Medium == "AcademicBook" || p.Medium == "AcademicArticle").length; i++) {
     if(dv.pages('"Book Log/2023"').where(p => p.Medium == "Book" || p.Medium == "eBook" || p.Medium == "GraphicNovel" || p.Medium == "AcademicBook" || p.Medium == "AcademicArticle" || p.Medium == p.Medium == "AcademicBook" || p.Medium == "AcademicArticle")[i].Length) {
       pagesum += dv.pages('"Book Log/2023"').where(p => p.Medium == "Book" || p.Medium == "eBook" || p.Medium == "GraphicNovel" || p.Medium == "AcademicBook" || p.Medium == "AcademicArticle")[i].Length;
         }
   }

let academicpages = 0;
   for(let i = 0; i < dv.pages('"Book Log/2023"').where(p => p.Medium == p.Medium == "AcademicBook" || p.Medium == "AcademicArticle").length; i++) {
     if(dv.pages('"Book Log/2023"').where(p => p.Medium == p.Medium == "AcademicBook" || p.Medium == "AcademicArticle")[i].Length) {
       academicpages += dv.pages('"Book Log/2023"').where(p => p.Medium == p.Medium == "AcademicBook" || p.Medium == "AcademicArticle")[i].Length;
         }
   }

let hoursum = 0;
	for(let i = 0; i < dv.pages('"Book Log/2023"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast").length;i++) {
     if(dv.pages('"Book Log/2023"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast")[i].Length) {
       hoursum += dv.pages('"Book Log/2023"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast")[i].Length;
         }
   }

let Grade = 0;
	for(let i = 0; i < dv.pages('"Book Log/2023"').length;i++) {
     if(dv.pages('"Book Log/2023"')[i].Rating) {
       Grade += dv.pages('"Book Log/2023"')[i].Rating;
         }
   }
let AvgRating = (Grade / (dv.pages('"Book Log/2023"').where(p => p.Medium).length))

let today = DateTime.now().toFormat("ooo");

dv.paragraph("#### This year, I've read " + BookCount + " books and I've listened to " + AudioCount + " audiobooks (of which " + dv.pages('"Book Log/2023"').where(p => p.Medium == "Podcast").length + " were podcasts), so " + (BookCount + AudioCount) + " regular books and audiobooks read. And I've read " + (AcademicCount) + " academic articles, monographs, etc, for a total " + (BookCount + AudioCount + AcademicCount) + " things read this year.")
...
dv.paragraph("#### On this day (Day " + today + " of 2023), I should have read " + (today * 50) + " pages, so I've read " + (pagesum - (today * 50)) + " pages more than the goal of 50 pages per day.")
